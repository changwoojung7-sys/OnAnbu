...
- **회원 탈퇴**:
  - **옵션 1**: '계정만 탈퇴' 시 아바타만 삭제하고 기존 안부 기록은 자녀들을 위해 보존.
  - **옵션 2**: '기록 포함 삭제' 시 본인이 작성한 모든 미디어와 데이터를 완전 파기.
...
- [x] **활동 내역**: 자녀가 보낸 안부 메시지 확인 (텍스트/사진). **실제 발신자 프로필 표시** (guardian 조회).
- [x] **알림 설정**: 부모님 설정 화면에서 알림 설정 링크 활성화 (`/notifications` 라우팅 연결).

### 🏠 자녀 대시보드 (Guardian Dashboard) - `app/(tabs)/index.tsx`

- [x] **홈 화면**: 등록된 부모님의 프로필과 상태 카드 표시. **화면 포커스 시마다 부모님 프로필 최신화** (`useFocusEffect` + `profiles` 테이블 재조회) 적용.
- [x] **안부 확인 (History)**: 활동 로그를 구글 포토 스타일의 **리치 미디어 카드 피드**로 통합 조회 (`two.tsx` 및 `HistoryFeed.tsx`).
  ...
  - **기분(Mood) 표시**: `daily_status` 테이블에서 mood 조회 → 안부 확인 기록에 기분 이모지 배지 표시.
- [x] **케어 기능 (Care)**: 구글 애드몹 보상형 광고 시청 후 사진/동영상/음성 통합 미디어 안부 발송 (`care.tsx`).
  - **미디어 타입 분기**: 사진은 `type='photo'`, 영상은 `type='video'`, 음성은 `type='voice_cheer'`로 DB에 저장.
  - **웹 환경 호환**: `ImagePicker`의 `allowsEditing`/`base64` 웹 비활성화, `fetch→blob` 업로드, `Haptics` 플랫폼 가드 적용.
  - **광고 대기 Mock**: 웹 환경에서 광고 로딩/리워드 대기 2초로 설정하여 대기 모달 노출.
- [x] **프로필 관리**: 이름 및 사진 수정 기능 (`profile/edit.tsx`). 실시간 상태 동기화 및 이미지 캐시 처리 완료.
- [x] **회원 탈퇴 (Withdrawal)**:
  - **주케어자**: 다단계 모달(Web 대응)을 통해 '계정만 탈퇴' 또는 '가족 전체 기록 파기' 선택 가능. Edge Function을 통한 Storage 일괄 정리 지원.
  - **케어대상**: '기록 보존형 탈퇴'와 '기록 완전 삭제형 탈퇴' 옵션 제공. 본인의 아바타 및 활동 로그 미디어 자동 삭제 로직 적용.

### 👨‍👩‍👧‍👦 가족 관리 (Family Management) - `app/family/index.tsx`

- [x] **가족 목록**: 현재 연결된 부모님과 보조 보호자 목록 확인. 한 명이 여러 명을 케어할 경우 **모든 초대 코드(#)** 표시 지원.
- [x] **부모님 추가**: 기존 가입 계정에서 초대 코드를 입력하여 즉시 다른 부모님과 연결하는 **"코드로 합류"** 기능 구현.
...
- **action_logs**: 모든 활동 로그 (기상, 미디어 전송, 메시지 등). `type` 컬럼 제약조건: `check_in`, `voice_cheer`, `message`, `photo`, `video`.
...
- [ ] **푸시 알림**: Supabase Realtime + Web Notifications API 기반 구현 준비 완료 (`lib/notificationService.ts`). Supabase Realtime 유료 구독 후 활성화 예정.
