...
- [x] **홈 화면**: 등록된 부모님의 프로필과 상태 카드 표시. 보조 보호자 로그인/가입 직후 정보가 누락되지 않도록 **자동 동기화 및 페칭 로직** 적용 완료 (`useFocusEffect` 연동).
...
- [x] **케어 기능 (Care)**: 보상형 광고 시청 후 사진/동영상/음성 통합 미디어 안부 발송 기능 구현 (`care.tsx`). **웹 환경 호환성**을 위해 `Haptics` 플랫폼 가드 적용 완료.
