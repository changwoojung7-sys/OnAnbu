...
| **Deployment** | **Cloudflare Pages** (`npm run build` → `expo export` + `404.html` 복사 SPA 방식) |
...
│   └── migrations/
├── scripts/                # 빌드 & 배포 스크립트
│   └── postbuild-cloudflare.js  # Cloudflare 후처리
├── zmd/                    # 프로젝트 문서
│   ├── 개발 지침서.md      # ← 이 문서
│   ├── Context.md          # 상세 구현 현황
│   ├── manual.md           # 사용자 메뉴얼
...
`npm run build`는 다음 2단계를 수행:

1. `npx expo export --platform web` → `dist/` 생성
2. `node scripts/postbuild-cloudflare.js` → 후처리

### postbuild-cloudflare.js 역할

| 작업 | 이유 |
|------|------|
| `assets/node_modules/` → `assets/_vendor/` 이동 | Cloudflare Pages가 `node_modules` 경로를 배포에서 제외 |
| JS 번들 내 `node_modules` → `_vendor` 참조 수정 | 폰트 경로 일치 |
| `index.html` → `404.html` 복사 | SPA 라우팅 지원 (`_redirects` 미사용) |

### 캐시 트러블슈팅

배포 후 커스텀 도메인에서 아이콘이 깨지거나 구 버전이 보이면:

1. **Cloudflare Dashboard** → `calamus.ai.kr` → Caching → **Purge Everything**
2. **브라우저** → `Ctrl + Shift + R` (강력 새로고침) 또는 캐시 삭제
3. `*.onanbu.pages.dev` 빌드 URL에서 먼저 정상 동작하는지 확인
